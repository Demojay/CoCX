...
use namespace CoC;
	...
	public class Aurora extends Follower
	{
		...
		{
			if (followerLevel >= 3 && HPRatio() < .4 && soulforce >= 100 && !hasStatusEffect(StatusEffects.MonsterVPT)) {
			...
			if (followerLevel >= 7 && followerLevel < 9) {
			...
			if (followerLevel >= 5 && followerLevel < 7) {
			...
			else if (followerLevel >= 3 && followerLevel < 5) {
		...
		public function Aurora()
		{
			...
			followerLevel = followerManager.followerLevels[FollowerManager.FOLLOWER_AURORA]["level"];
			...
			if (followerLevel < 5) this.lustVuln = .5;
			if (followerLevel >= 5 && followerLevel < 8) this.lustVuln = .4;
			if (followerLevel >= 8 && followerLevel < 13) this.lustVuln = .3;
			if (followerLevel >= 13 && followerLevel < 18) this.lustVuln = .2;
			if (followerLevel >= 18) this.lustVuln = .1;
			this.a = "";
			this.short = "Aurora";
		...
		public function stunAttack(monster:Monster, display:Boolean = true):void {
			...
			functions.doDamageFollower(this, dmg3a, true, display);
